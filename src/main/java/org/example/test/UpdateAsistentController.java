...
public class UpdateAsistentController implements Initializable {
    ...
    @FXML
    private ChoiceBox<String> tipFld;
    ...
    void modificaButton(ActionEvent e){
        if(numeFld.getText().isBlank() || prenumeFld.getText().isBlank() ||
                ...
                tipFld.getValue() == null || gradFld.getValue() == null){
            ...
            try {
                ...
                if(exista == 0){
                    ...
                    String updateAsistent = "{call UpdateAsistent(?, ?, ?)}";
                    callableStatement = connection.prepareCall(updateAsistent);
                    callableStatement.setInt(1, angajatID);
                    callableStatement.setString(2, String.valueOf(tipFld.getValue()));
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try{
            ...
            String asistentInfo = "SELECT * FROM asistentmedical WHERE ID_Angajat = " + angajatID;
            resultSet = statement.executeQuery(asistentInfo);
            while(resultSet.next()){
                tipFld.setValue(resultSet.getString("TipAsistent"));
                gradFld.setValue(resultSet.getString("GradAsistent"));
            }
        }catch (Exception ex){
            ex.printStackTrace();
        }
